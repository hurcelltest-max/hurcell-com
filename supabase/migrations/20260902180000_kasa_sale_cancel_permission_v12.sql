-- ============================================================================
-- Migration: 20260902180000_kasa_sale_cancel_permission_v12.sql
-- Description: HurCELL Kasa V12 - Kullanıcı Bazlı Satış İptal Yetkisi (Bahar AYDAMGA),
--              Tekil 5-Parametreli fn_kasa_cancel_sale Kanonik RPC Sözleşmesi
...
-- ============================================================================
-- 1. KULLANICI YETKİ TABLOSU (KASA_USER_PERMISSIONS)
-- ============================================================================
CREATE TABLE IF NOT EXISTS public.kasa_user_permissions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES public.kasa_users(id) ON DELETE CASCADE,
    permission_key TEXT NOT NULL,
    is_allowed BOOLEAN NOT NULL DEFAULT true,
    granted_by_user_id UUID REFERENCES public.kasa_users(id),
    granted_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    revoked_at TIMESTAMPTZ DEFAULT NULL,
    CONSTRAINT uq_kasa_user_permissions_user_key UNIQUE (user_id, permission_key)
);

CREATE INDEX IF NOT EXISTS idx_kasa_user_permissions_active
ON public.kasa_user_permissions (user_id, permission_key)
WHERE is_allowed = true AND revoked_at IS NULL;

ALTER TABLE public.kasa_user_permissions ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON TABLE public.kasa_user_permissions FROM PUBLIC, anon, authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.kasa_user_permissions TO service_role;

-- ============================================================================
-- 2. KESİN BAHAR UUID DOĞRULAMA VE YETKİ TANIMLAMA (FAIL-CLOSED DO BLOCK)
-- ============================================================================
DO $$
DECLARE
    ...
    v_admin_id UUID;
BEGIN
    -- 1. Hedef kullanıcının UUID, username, full_name, role ve aktifliğini kesin olarak doğrula
    ...
    IF v_target_user.username <> 'bahar' OR v_target_user.full_name <> 'Bahar AYDAMGA' THEN
    ...
    FROM public.kasa_users
    WHERE role = 'yonetici' AND is_active = true
    ORDER BY created_at ASC
    LIMIT 1;

    -- 3. Bahar kullanıcısına 'kasa.sale.cancel' yetkisi tanımla (Rolü personel olarak kalır)
    INSERT INTO public.kasa_user_permissions (
        user_id, permission_key, is_allowed, granted_by_user_id, granted_at
    ) VALUES (
        c_target_uuid, 'kasa.sale.cancel', true, v_admin_id, now()
    )
    ...
    INSERT INTO public.kasa_audit_logs (
        ...
        jsonb_build_object(
            ...
            'permission_key', 'kasa.sale.cancel'
        ),
        'HurCELL Kasa V12 - Bahar AYDAMGA Satış İptal Yetkisi Tanımlandı'
    );
END $$;

-- ============================================================================
-- 3. KANONİK TEKİL 5-PARAMETRELİ FN_KASA_CANCEL_SALE FONKSİYONU
-- ============================================================================
...
BEGIN
    ...
    -- 9. Satış Durumunu 'cancelled' Olarak Güncelle
...
-- ============================================================================
-- 4. ACL VE GÜVENLİK SIKILAŞTIRMASI (TEKİL 5-PARAMETRELİ KANONİK İMZA)
-- ============================================================================
