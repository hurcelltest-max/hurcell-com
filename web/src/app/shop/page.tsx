  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 w-full py-6 space-y-6 flex-1">
      ...
      <div className="border-b border-slate-200 pb-5">
        ...
        <p className="text-xs text-slate-500 mt-1">Geniş teknoloji ürün yelpazemizi ve güncel stoklarımızı inceleyin.</p>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-3xl p-5 space-y-5 shadow-sm">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
          ...
          <div className="lg:col-span-5 relative">
            ...
            {searchQuery && (
              <button onClick={() => setSearchQuery('')} className="absolute right-4 top-3.5 text-slate-450 hover:text-slate-800">
                <X size={16} />
              </button>
            )}
          </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 rounded-3xl border border-dashed border-slate-200 bg-white/50 space-y-3 shadow-sm">
          ...
          <p className="text-xs text-slate-400 max-w-xs mx-auto">Arama terimlerinizi veya filtrelerinizi sıfırlayarak tekrar aramayı deneyebilirsiniz.</p>
          <button
            onClick={resetFilters}
            className="px-5 py-2.5 bg-slate-100 hover:bg-slate-250 border border-slate-200 text-slate-800 text-xs font-semibold rounded-xl transition-colors cursor-pointer"
          >
            Filtreleri Temizle
          </button>
        </div>
              ...
              return (
                <div key={product.id} className="group flex flex-col h-full bg-white hover:shadow-md border border-slate-200/80 hover:border-slate-300 rounded-3xl p-4 transition-all duration-350 hover:-translate-y-1 shadow-sm">
                  ...
                  <div className="flex flex-col flex-1">
                    ...
                    {/* Category Label */}
                    <span className="text-[10px] uppercase tracking-wider font-mono text-slate-550 mb-1">
                      {product.category || 'Teknoloji'}
                    </span>

                    {/* Product Name */}
                    <h3 className="text-sm font-bold text-slate-800 mb-1.5 leading-snug line-clamp-2" title={product.name}>
                      {product.name}
                    </h3>
...
export default function ShopPage() {
  return (
    <div className="min-h-screen bg-slate-50 text-slate-850 font-sans pt-28 pb-10 flex flex-col">
      <Suspense fallback={
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 w-full py-6 space-y-6 flex-1">
